...
const PurchaserReport = () => {
   ...
   useEffect(() => {
      try {
         const fetchData = async () => {
            //setTransactions(await getPurchaserTransactionsBydate(convertOnlyDate(new Date()))
            setTransactions([
               ...(await getPurchaserTransactionsBydate(convertOnlyDate(new Date()))),
            ]);
            setIsLoading(false);
         };
         fetchData();
   ...
   const dateChange = async (e) => {
      ...
      setTransactions([...(await getPurchaserTransactionsBydate(e.target.value))]);
      setIsLoading(false);
   };
   const monthChange = async (e) => {
      ...
      setTransactions([...(await getPurchaserTransactionsByMonth(e.target.value))]);
      setIsLoading(false);
   };
   const submit = () => {};
   const handleRadioChange = async (e) => {
      ...
      if (e.target.name === "bydate") {
         ...
         setTransactions([
            ...(await getPurchaserTransactionsByMonth(getOnlyMonth(new Date()))),
         ]);
         setIsLoading(false);
        ...
        setTransactions([
            ...(await getPurchaserTransactionsBetweenDates(convertOnlyDate(new Date()), convertOnlyDate(ed)))
         ]);
         setIsLoading(false);
      }
   };
   ...
   useEffect(() => {
      ...
      const fetchData = async () => {
         if (new Date(startDate) <= new Date(endDate) && !isInit) {
            ...
            ed = new Date(ed.setDate(ed.getDate() + 1));
            setTransactions([
               ...(await getPurchaserTransactionsBetweenDates(startDate, ed)),
            ]);
            setIsLoading(false);
         }
      };
        ...
        <div>
         ...
         {isLoading ? (
            ...
            <div>
               ...
               />
               <PurchaserReportTxnTable transactionSummary={transactions} />
            </div>
         ) : (
